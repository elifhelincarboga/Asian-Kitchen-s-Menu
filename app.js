...
function createFood (food) {
  ...
  el.classList.add("menu-items", "col-lg-6", "col-sm-12")
  el.innerHTML = `
...
function getMenu (category) {
  ...
  if (category === 'All') {
    ...
    result = menu.filter(food => food.category === category)
  }
  return result
}
...
function createMenu (category) {
  let menuFiltered = getMenu(category)
  const element = document.querySelector(".section-center")
  element.querySelectorAll('*').forEach(n => n.remove());
  for (let i = 0; i < menuFiltered.length; i++ ){
    let food = createFood(menuFiltered[i])
